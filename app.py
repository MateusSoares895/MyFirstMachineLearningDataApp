...
def train_model():
    ...
    rf_regressor = RandomForestRegressor(
        n_estimators=200, max_depth=7, max_features=3)
...
faixa_valores = st.slider("Faixa de preço", float(
    data.MEDV.min()), 150., (10.0, 100.0))

# filtrando os dados
dados = data[data['MEDV'].between(
    left=faixa_valores[0], right=faixa_valores[1])]
...
indus = st.sidebar.number_input(
    "Proporção de Hectares de Negócio", value=data.CRIM.mean())
...
ptratio = st.sidebar.number_input(
    "Índice de alunos para professores", value=data.PTRATIO.mean())

b = st.sidebar.number_input(
    "Proporção de pessoas com descendencia afro-americana", value=data.B.mean())

lstat = st.sidebar.number_input(
    "Porcentagem de status baixo", value=data.LSTAT.mean())
